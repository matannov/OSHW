...
struct runqueue {
	...
	task_t *curr, *idle;
	prio_array_t *active, *expired, *ontime_short, *overdue_short, arrays[4];
...
#define rt_pol(p)		((p)->policy == SCHED_FIFO || (p)->policy == SCHED_RR)
#define short_pol(p)	((p)->policy == SCHED_SHORT && (p)->trial_num)
#define overdue_pol(p)	((p)->policy == SCHED_SHORT && !(p)->trial_num)
#define other_pol(p)	((p)->policy == SCHED_OTHER)
#define same_pol(a,b)	((rt_pol(a) && rt_pol(b)) || (short_pol(a) && short_pol(b)) || (overdue_pol(a) && overdue_pol(b)) || (other_pol(a) && other_pol(b)))
...
repeat_lock_task:
	...
	if (!p->array) {
		 ...
		 */
		if ((same_pol(p,rq->curr) && p->prio < rq->curr->prio) ||
			(other_pol(p) && overdue_pol(rq->curr)) ||
			(overdue_pol(p) && rq->curr->pid == 0) || 
			(short_pol(p) && (other_pol(rq->curr) || overdue_pol(rq->curr))) ||
			(rt_pol(p) && !rt_pol(rq->curr)))
...
void wake_up_forked_process(task_t * p)
{
	...
	if (overdue_pol(current) && (current->array == rq->ontime_short)) {
		dequeue_task(current, rq->ontime_short);
		current->prio = 0;
		enqueue_task(current, rq->overdue_short);
	}
	if (short_pol(current)) {
		dequeue_task(current, rq->ontime_short); 
		enqueue_task(current, rq->ontime_short);
	}
...
void scheduler_tick(int user_tick, int system)
{
	...
	if (p->array != rq->active && !short_pol(p) && !overdue_pol(p)) {
		set_tsk_need_resched(p);
		return;
	}
	spin_lock(&rq->lock);
	...
	if (p->sleep_avg)
		p->sleep_avg--;
	if (short_pol(p)) {
		dequeue_task(p, rq->ontime_short);
		set_tsk_need_resched(p);
		if (!--p->time_slice) {
			if (!--p->trial_num) {
				p->prio = 0;
				...
				if (!p->time_slice) {
					...
					enqueue_task(p, rq->ontime_short);
				}
			}
		}	
	} /* DANGER! DANGER! */
...
pick_next_task:
	...
	if ((rq->active->nr_active) || (rq->expired->nr_active)) {
		...
		idx = sched_find_first_bit(array->bitmap);
		if (idx > MAX_RT_PRIO && (rq->ontime_short->nr_active)) {
			array = rq->ontime_short;
			idx = sched_find_first_bit(array->bitmap);
		}
	}
...
static int setscheduler(pid_t pid, int policy, struct sched_param *param)
{
	...
	if ((param->requested_time < 1) || (param->requested_time > 5000) ||
